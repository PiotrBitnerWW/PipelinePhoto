...
class CameraViewController: UIViewController {
  ...
  @IBOutlet private weak var zoomSlider: VerticalSlider!
  
  @IBAction func cancelAction(sender: UIButton) { cancel() }
  func cancel() { self.dismiss(animated: true) {} }
 ...
 */
   ...
   // captureModeControl.isEnabled = false
    ...
    sessionQueue.async {
      ...
      //  self.recordButton.isEnabled = self.movieFileOutput != nil
       ...
       self.toggleFlashButton.isEnabled =  (currentPosition == .front)
       self.zoomSlider.isEnabled =  (currentPosition == .front)
  ...
  @IBAction func toggleFlash(_ sender: UIButton) {
    if self.flashMode == .on {
      self.flashMode = .off
      toggleFlashButton.setImage(UIImage(named: "Flash_Disabled"), for: .normal)
    }
      
    else {
      self.flashMode = .on
      toggleFlashButton.setImage(UIImage(named: "Flash"), for: .normal)
    }
  }
  ...
  func setupZoom() {
    ...
    //let device = self.videoDeviceInput?.device
    //let maxValue = Float( device?.maxAvailableVideoZoomFactor ?? 100.0)
    self.zoomSlider.maximumValue = 5.0 //maxValue/10.0
    
    self.zoomSlider.minimumValue = 1.0
    self.zoomSlider.addTarget(self, action: #selector(CameraViewController.zoom(_:)), for: .valueChanged)
  }
  
  @objc
  func zoom(_ sender: UISlider) {
    let device = self.videoDeviceInput.device
  ...
  @IBAction private func capturePhoto(_ photoButton: UIButton) {
    ...
    sessionQueue.async {
      ...
      */
       ...
       let jpegQuality =  NSNumber(floatLiteral: 1.0)
      photoSettings = AVCapturePhotoSettings(format: [AVVideoCodecKey: AVVideoCodecType.jpeg, AVVideoCompressionPropertiesKey: [AVVideoQualityKey: jpegQuality]  ])
      ...
      let photoCaptureProcessor = PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: {
       ...
       //     self.inProgressLivePhotoCapturesCount -= 1
        ...
        self.sessionQueue.async {
         ...
         self.delegate?.photoTaken(data:  photoCaptureProcessor.photoData)
